import React from 'react';
import { Grid } from '@material-ui/core';
import './Stats.scss'

const StatsItem = (props)=> {
    return(
        <Grid item xs={12} sm={6} m={4} className="stat-item">
            <p className="stat-item__name">
                {props.name}
            </p>
            <p className="stat-item__number">
                {props.number}
            </p>
        </Grid>
    );
}

const Stats = () => {
    return (
        <Grid
            container
            direction="row"
            justify="spaceEvenly"
            alignItems="center"
            spacing={0}
            xs={12}
        >
            <Grid container item xs={12} justify="center">
                <img className="profile-image" src="data:image/png;base64,"></img>
            </Grid>
                <StatsItem name="Время, проведённое в игре" number="3"></StatsItem>
                <StatsItem name="Потраченные деньги" number="120"></StatsItem>
                <StatsItem name="Выигранные деньги" number="120"></StatsItem>
                <StatsItem name="Недоспанные часы" number="540"></StatsItem>
                <StatsItem name="Разочарованные родственники" number="2"></StatsItem>
                <StatsItem name="Разбитые чашки" number="1"></StatsItem>
        </Grid>
    );
}

export default Stats;